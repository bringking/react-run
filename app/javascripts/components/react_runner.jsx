...
class ReactRunner extends React.Component {
  ...
  onLoopDetected = line => {
    this.setState({
      loopError: `<h1>Infinite loop detected around line: ${line - 18}</h1>
    <p>React.run has shortcut this loop to allow you to continue working.<p>
    <p>To remove this protection add this line your code '//react-run no-loop-protect'</p>
  ...
  renderCode = code => {
    ...
    if (frame) {
      if (this.state.loopError) {
        this.setState({ loopError: null }, () => {
          renderReactToFrame(frame, code, this.onLoopDetected);
        });
      } else {
        renderReactToFrame(frame, code, this.onLoopDetected);
      }
      ...
      <div className="app-container">
        <div className='app-inner'>
          ...
          <div id="results">
            <Errors
              socket={this.socket}
              loopError={this.state.loopError}
